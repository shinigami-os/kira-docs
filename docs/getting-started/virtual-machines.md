...
Switch to the plain, non-Secure-Boot firmware: in virt-manager, the Overview tab's Firmware dropdown has a variant without `.ms.` or `.secboot.` in its path (`OVMF_CODE_4M.fd`). If the dropdown refuses the change with `Unable to find 'efi' firmware that is compatible with the current configuration`, the domain XML still has the old `<firmware><feature enabled='yes' name='secure-boot'/>` requirement — edit the XML directly and disable both `secure-boot` and `enrolled-keys` features, point `<loader>`/`<nvram>` at the plain (non-`.ms.`) OVMF files, and delete the existing NVRAM file under `/var/lib/libvirt/qemu/nvram/` so it regenerates cleanly from the new template rather than keeping boot entries left over from the Secure Boot config.
